...
bool Shader::readFile(const char filename[])
{
    ...
    while(!feof(fp)){
        ...
        switch(Input){
        case 'E':{
			fscanf_s(fp, "%f %f %f", &eyes_position[0], &eyes_position[1], &eyes_position[2]);
        }break;
        case 'V':{
			fscanf_s(fp, "%f %f %f", &view_direction[0], &view_direction[1], &view_direction[2]);
        }break;
        case 'L':{
			fscanf_s(fp, "%f %f %f", &light_position[0], &light_position[1], &light_position[2]);
        }break;
        case 'F':{
			fscanf_s(fp, "%lf", &field_of_view);
        }break;
        case 'R':{
			fscanf_s(fp, "%d %d", &resolution_width, &resolution_height);
        }break;
        case 'S':{
            Sphere stmp;
			fscanf_s(fp, "%f %f %f %f", &stmp.center[0], &stmp.center[1], &stmp.center[2], &stmp.radius);
            stmp.material = mat;
            addSphere(&sphere_list, stmp);
        }break;
        case 'T':{
            Triangle ttmp;
			fscanf_s(fp, "%f %f %f", &ttmp.vertex1[0], &ttmp.vertex1[1], &ttmp.vertex1[2]);
			fscanf_s(fp, "%f %f %f", &ttmp.vertex2[0], &ttmp.vertex2[1], &ttmp.vertex2[2]);
			fscanf_s(fp, "%f %f %f", &ttmp.vertex3[0], &ttmp.vertex3[1], &ttmp.vertex3[2]);
            ttmp.material = mat;
        ...
        case 'M':{
			fscanf_s(fp, "%f %f %f", &mat.r, &mat.g, &mat.b);
			fscanf_s(fp, "%f %f %f %f", &mat.Ka, &mat.Kd, &mat.Ks, &mat.exp);
            fscanf_s(fp, "%f %f %f", &mat.Reflect, &mat.Refract, &mat.Nr);
        }break;
        }
    }
    fclose(fp);
...
void Shader::outputPPMformat(const char filename[])
{
	...
	global_image.init(resolution_width, resolution_height);
	
    for(int i = 0; i < resolution_height; i++){

		rows[i]=new thread(threading, i,ray,resolution_width, view_direction, screen_width, pixel_length, screen_lift,screen_height, screen_up);
        ...
        /*for(int j = 0; j < resolution_width; j++){
            ...
            Tracing(ray, &color, 1);
			global_image.writePixel(j,i,color);
            color = {0,0,0};
	...
	global_image.outputPPM(filename);
}


void threading(int i, Ray ray,int resolution_width, vec3 view_direction, double screen_width, double pixel_length, vec3 screen_lift, double screen_height, vec3 screen_up)
{
